...
- **Humpback-associated** recordings appear **weaker and noisier** on DAS under current preprocessing; interpretation remains **support-level** and should not be equated with Orca-class clarity. The **same** selected-channel/audio **UI and export pattern** as Orca is used in the viewer so exploration is not a reduced mode; claims about signal quality remain data-driven in captions and JSON interpretation fields.
