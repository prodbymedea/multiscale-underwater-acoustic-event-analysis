...
- `das_waterfall_preview.npz` — teacher-style raw/native waterfall artifact used by the viewer **DAS Waterfall** mode when available. Current keys are `data` `[time, channels]` (`int16`, native DAS counts before `AmpScaling`), `t_s`, `sample_indices`, `channel_indices`, `distances_m`, `raw_fs_hz`, `fs_hz`, `time_downsample`, and `amp_scaling`. Default export matches the teacher notebook preview: first 30,000 raw DAS samples, all 946 channels, no filtering, no normalization, no `AmpScaling`.
...
3. **DAS waterfall** — `files.das_waterfall_preview`, preferably `das_waterfall_preview.npz`, rendered as raw/native amplitude over time sample index and channel index. This is useful for cable-wide structure and non-whale wave-like patterns; it is **not** a validated detector. If the teacher-style artifact is missing, the frontend can fall back to `das_preprocessed_preview.npz`, but that preview is channel-subsampled and normalized, so it will not visually match the teacher notebook.
